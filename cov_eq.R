...
simulation <- function(nsamp, truebeta, minnumvis, sampsize, seed){
  ...
  for (s in 1:nsamp){
    ...
    ### set up data frame
    dat <- data.frame(naccid=rep(1:n, times=p), vnumber=obs)
    naccid <- unique(dat$naccid)
    group<-ifelse(rbinom(length(naccid),1,.5)==0,1,2)
    ...
    dat <- dat %>% rename(obs=vnumber) %>% group_by(naccid)%>% mutate(vnumber=row_number()) %>% ungroup()
    dat <- data.frame(dat)
    ...
    dat$logimem_new <- (beta0 + rep(U[,1], times=p)) + (beta1 + rep(U[,2], times=p)) * dat$vnumber * dat$group + dat$eij
    dat<-data.frame(dat)
...
sim.results<-simulation(10, 5, 9, 60, 12163)
write.csv(sim.results, "test1cov.csv")

sim.results<-simulation(10, 5, 9, 60, 121643)
write.csv(sim.results, "test2cov.csv")


sim.results<-simulation(1000, -5, 9, 60, 76)
write.csv(sim.results, "N60_9vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 9, 60, 47)
write.csv(sim.results, "N60_9vis_betan05_cov_eq.csv")



sim.results<-simulation(1000, -5, 5, 60, 1223513)
write.csv(sim.results, "N60_5vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 5, 60, 4534)
write.csv(sim.results, "N60_5vis_betan05_cov_eq.csv")


sim.results<-simulation(1000, -5, 3, 60, 121553)
write.csv(sim.results, "N60_3vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 3, 60, 43255)
write.csv(sim.results, "N60_3vis_betan05_cov_eq.csv")



sim.results<-simulation(1000, -5, 9, 100, 1213523)
write.csv(sim.results, "N100_9vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 9, 100, 4255)
write.csv(sim.results, "N100_9vis_betan05_cov_eq.csv")


sim.results<-simulation(1000, -5, 5, 100, 1212453)
write.csv(sim.results, "N100_5vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 5, 100, 4255)
write.csv(sim.results, "N100_5vis_betan05_cov_eq.csv")


sim.results<-simulation(1000, -5, 3, 100, 1212343)
write.csv(sim.results, "N100_3vis_betan5_cov_eq.csv")

sim.results<-simulation(1000, -0.5, 3, 100, 45253)
write.csv(sim.results, "N100_3vis_betan05_cov_eq.csv")